...
class MyLikesRepositoryImpl implements MyLikesRepository {
  ...
  @override
  Future<Result<LikedRoutePage>> fetchLikedRoutes({int? cursor, int size = 10}) async {
    try {
      final response = await _service.fetchLikedRoutes(cursor: cursor, size: size);
      return Result.success(
        LikedRoutePage(items: response.content, nextCursor: response.nextCursor, hasNext: response.hasNext),
      );
    } catch (error) {
  ...
  @override
  Future<Result<LikedBoulderPage>> fetchLikedBoulders({int? cursor, int size = 10}) async {
    try {
      final response = await _service.fetchLikedBoulders(cursor: cursor, size: size);
      return Result.success(
        LikedBoulderPage(items: response.content, nextCursor: response.nextCursor, hasNext: response.hasNext),
      );
    } catch (error) {
      return Result.failure(AppFailure.fromException(error));
    }
